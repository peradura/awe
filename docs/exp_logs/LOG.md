...
| 2026-07-06 | `reachp.py` 라벨링 수정 + 재실행 | `ans`가 현재 쿼리 공개분을 무시(모델은 retrieval 전에 write) → 정답 라벨 교정 후 동일 시드 재실행 (`ans` 의존 통계만 변화) | ⏳ 재실행 — corr·depth표 재측정 | `results/reachp_run_v2.log`, `results/reachp_curve_v2.png` |
