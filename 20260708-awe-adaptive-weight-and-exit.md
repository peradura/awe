...
## 6. 실험 결과 (모든 수치 10 seeds 평균; 원본: AWE 레포 `docs/RESULTS.md`)
...
**왜 이렇게 갈리나 (쓰기 vs 멈춤).** 두 손잡이는 매 latent step에서 **하는 일이 다르다**:
- **쓰기(Weight = TTT write)** = 메모리 $\delta$를 *얼마나 고칠지*(갱신 세기). surprise(‖Wk−v‖²)가 큰 만큼 크게 갱신 = *틀린 만큼 쓴다*(error-gated) → **surprise가 옳은 기준** ✅.
- **멈춤(Exit = halting)** = 루프를 *언제 그만둘지*(깊이). 문제는 surprise가 낮다(=메모리가 지금 맞다)고 **생각이 다 끝난 건 아니라는 것** — multi-hop에선 정답 경로를 덜 밟고도 surprise가 낮아, 이를 멈춤 기준으로 쓰면 **확신하고 틀린 채 조기 종료**. 그래서 멈춤엔 *예측이 실제로 안 움직이나*(**수렴**)를 봐야 한다.
