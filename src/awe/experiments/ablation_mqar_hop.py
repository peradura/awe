"""Multi-hop MQAR: the signal-discriminating transfer test.

Same halting bake-off as `ablation_mqar` but on H-hop associative recall
(`datasets/mqar_hop.py`), which restores a depth∝difficulty structure so that
halting too early is costly again. The question: does reachp's discrimination
(entropy/recon lose, conv/dstate don't) reproduce here — on an external
associative-recall task rather than reachability?

Reuses the reachp3 harness (trace_stream / apply_halt / tau_grid / decompose,
held-out tau, failure decomposition). Reports depth-vs-K (does the halter spend
more steps on higher-hop probes?).
...
from awe.datasets.mqar_hop import make_batch, HopConfig
...
def train(model, cfg, opt, device, steps, batch, rng, lam_aux, log_every=400):
    """Chase-and-hold supervision (traj[:, t] = g^{min(t+1,H)}(q)) + aux read-back."""
    model.train()
...
def run_seed(args, device):
    ...
    print(f"seed={args.seed} device={device} | params="
          ...
          f"n={cfg.n} m={cfg.m} Q={cfg.Q} T={cfg.T} Hmax={cfg.Hmax}")

    print("== train (multi-hop chase-and-hold, persist stream) ==")
    ...
    for name in SIGNALS:
        ...
        sig0 = tr_ev[name][:, t0]; m = torch.isfinite(sig0)
    ...
    # depth vs difficulty K=H (on answerable & correct): graded depth?
    hs_best = apply_halt(tr_ev["preds"], tr_ev["dstate"], tr_ev["tgt"],
                         out["arms"]["dstate"]["tau"])[2] + 1
    Kt = tr_ev["K"]; ok = (tr_ev["ans"] > 0.5) & (tr_ev["preds"][:, -1] == tr_ev["tgt"])
    dvk = {}
    for k in sorted(set(int(x) for x in Kt.tolist())):
        mk = ok & (Kt == k)
        if mk.sum() >= 15:
            dvk[str(k)] = hs_best[mk].float().mean().item()
    out["depth_vs_K"] = dvk
    print(f"  depth vs K (dstate, ans&correct): {dvk}")

    path = f"{args.out}/mqarhop_seed{args.seed}.json"
...
def aggregate(args):
    files = sorted(glob.glob(f"{args.out}/mqarhop_seed*.json"))
    ...
    for name in SIGNALS:
        ...
        print(f"  {name:8s} | acc {acc.mean()*100:5.1f}±{acc.std(unbiased=False)*100:.1f}%"
              f" | steps {st.mean():4.2f}±{st.std(unbiased=False):.2f} | gap {gap.mean():+5.1f}pp"
              f" | early-wrong {ew.mean()*100:.0f}%")
    winner = max(SIGNALS, key=lambda n: sum(r["arms"][n]["acc"] for r in runs))
    print(f"== best-accuracy arm = {winner} (ties within noise; see gaps) ==")
    ...
    plt.xlabel("avg steps used"); plt.ylabel("multi-hop MQAR accuracy (%)")
    plt.title(f"Multi-hop MQAR halting bake-off (seed {r0['seed']}; aggregate in log)")
    plt.legend(fontsize=8); plt.grid(alpha=.3); plt.tight_layout()
    fig = f"{args.out}/mqarhop_bakeoff.png"
